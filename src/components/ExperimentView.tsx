...
export const ExperimentView: React.FC<ExperimentViewProps> = ({ workspaceId }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Carregando experimentos...</div>
      ) : experiments.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-12 text-center">
          <FlaskConical className="w-10 h-10 text-slate-600 mx-auto mb-3" />
          <h3 className="text-sm font-semibold text-white mb-1">Nenhum experimento cadastrado</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {experiments.map((exp) => {
            ...
            return (
              <div key={exp.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
                ...
                {exp.methodology && (
                  <div className="text-xs text-slate-400">
                    <strong className="text-slate-300">Metodologia:</strong> {exp.methodology}
                  </div>
                )}

                {exp.sample_size && (
                  <div className="text-xs text-slate-400">
                    <strong className="text-slate-300">Amostra:</strong> {exp.sample_size} usuários
                  </div>
                )}
                ...
                {exp.learnings && (
                  <div className="bg-indigo-950/30 p-3 rounded-lg border border-indigo-500/20 text-xs">
                    <span className="font-bold text-[10px] text-indigo-300 uppercase tracking-wider block mb-1">
                      Aprendizados do Produto
                    </span>
                    <p className="text-indigo-100">{exp.learnings}</p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                  {exp.status === 'draft' && (
                    <button
                      ...
                      className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded flex items-center gap-1"
                    >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-lg w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-4">Novo Experimento</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Hipótese a Testar</label>
                <select
                  value={hypothesisId}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Título do Experimento</label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Metodologia</label>
                <input
                  type="text"
                  value={methodology}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Tamanho da Amostra (Usuários)</label>
                <input
                  type="number"
                  value={sampleSize}
      ...
      {editingExp && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-lg w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-2">Concluir Experimento & Registrar Aprendizados</h3>
            <p className="text-xs text-slate-400 mb-4">{editingExp.title}</p>
            <form onSubmit={handleSaveResults} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Resultados Obtidos</label>
                <textarea
                  value={resultsText}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Aprendizados Chave</label>
                <textarea
                  value={learningsText}
