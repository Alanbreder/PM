...
export const DecisionView: React.FC<DecisionViewProps> = ({ workspaceId }) => {
  ...
  const getStatusBadge = (st: string) => {
    switch (st) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-700/50 text-slate-300 border border-slate-600">
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-500/30 rounded-xl p-6 shadow-md text-slate-100">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-400">• Product OS</span>
            </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs rounded-lg transition shadow-sm whitespace-nowrap"
          >
      ...
      {/* Decisions List */}
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Carregando decisões do produto...</div>
      ) : decisions.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-12 text-center">
          <div className="w-12 h-12 rounded-full bg-indigo-500/10 text-indigo-400 flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 className="w-6 h-6" />
          </div>
          <h3 className="text-sm font-semibold text-white mb-1">Nenhuma Decisão Registrada</h3>
          ...
          <button
            onClick={() => setShowModal(true)}
            className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium rounded-lg transition"
          >
            <Plus className="w-4 h-4" />
            ...
            return (
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      ...
                      <h3 className="text-base font-bold text-white leading-snug">{dec.title}</h3>
                    </div>
                    {getStatusBadge(dec.status)}
                  </div>

                  {dec.description && (
                    <p className="text-xs text-slate-300 mb-3">{dec.description}</p>
                  )}
                  ...
                  <div className="bg-slate-950/80 rounded-lg p-3 border border-slate-800/80 mb-3">
                    ...
                    <p className="text-xs font-medium text-slate-200">{dec.decision}</p>
                  </div>

                  {/* Rationale */}
                  {dec.rationale && (
                    <div className="mb-3 text-xs text-slate-400">
                      <span className="font-semibold text-slate-300">Racional: </span>
                      {dec.rationale}
                    </div>
                  )}
                  ...
                  {exp && (
                    <div className="mt-3 pt-3 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        ...
                        <span className="truncate">Experimento: <strong className="text-slate-300">{exp.title}</strong></span>
                      </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between">
                  <span className="text-[10px] text-slate-500">ID: {dec.id.substring(0, 8)}</span>
                  <button
            ...
            <form onSubmit={handleCreateDecision} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  required
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"
                >
