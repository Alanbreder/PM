...
export const ResearchView: React.FC<ResearchViewProps> = ({ workspaceId }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
        <div>
      ...
      {loading ? (
        <div className="p-12 text-center text-zinc-400 text-sm">Carregando pesquisas...</div>
      ) : researches.length === 0 ? (
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-sm font-semibold text-zinc-100 mb-1">Nenhuma pesquisa registrada</h3>
          ...
          {researches.map((r) => (
            <div key={r.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800 pb-3">
                <div>
                  <h3 className="text-base font-bold text-zinc-100">{r.title}</h3>
                  {r.target_audience && (
                    <span className="text-[11px] text-zinc-400">Público-alvo: {r.target_audience}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2.5 py-1 rounded text-[10px] font-semibold uppercase tracking-wider ${
                      r.status === 'analyzed'
                        ...
                        : 'bg-military-500/20 text-military-300 border border-military-500/30'
                    }`}
                  >
                  ...
                  {r.raw_notes && r.status !== 'analyzed' && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-military-900/80 text-military-300 hover:bg-military-800/80 border border-military-700/50 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition disabled:opacity-50"
                    >
              ...
              {r.objective && <p className="text-xs text-zinc-300"><strong>Objetivo:</strong> {r.objective}</p>}

              {r.raw_notes && (
                <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800/80 text-xs text-zinc-300 max-h-32 overflow-y-auto whitespace-pre-wrap">
                  <span className="font-bold text-[10px] text-zinc-400 uppercase tracking-wider block mb-1">Anotações Brutas</span>
                  {r.raw_notes}
                </div>
              )}
              ...
              {r.key_findings && r.key_findings.length > 0 && (
                <div className="bg-military-950/60 border border-military-700/50 p-4 rounded-lg space-y-2">
                  <h4 className="text-xs font-bold text-military-300 flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-military-400" />
                    Principais Descobertas da IA
                  </h4>
                  <ul className="list-disc list-inside text-xs text-military-200 space-y-1">
              ...
              {r.suggested_problems && r.suggested_problems.length > 0 && r.status !== 'analyzed' && (
                <div className="bg-zinc-950 border border-military-500/30 p-4 rounded-lg space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-military-300">Problemas Sugeridos pela IA</h4>
                    <button
                      onClick={() => handleApproveAnalysis(r)}
                      className="px-3 py-1 bg-military-600 hover:bg-military-500 text-zinc-100 text-xs font-semibold rounded flex items-center gap-1 transition"
                    >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {r.suggested_problems.map((p, i) => (
                      <div key={i} className="p-3 bg-zinc-900 rounded border border-zinc-800 text-xs space-y-1">
                        <div className="font-bold text-zinc-100">{p.title}</div>
                        <div className="text-zinc-300">{p.description}</div>
                        <div className="text-[10px] text-military-400 font-medium">Impacto: {p.impact}</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 max-w-lg w-full text-zinc-100">
            <h3 className="text-base font-bold text-zinc-100 mb-4">Nova Pesquisa de Discovery</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Objetivo</label>
                <input
                  type="text"
                  value={objective}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Anotações Brutas / Transcrição</label>
                <textarea
                  value={rawNotes}
