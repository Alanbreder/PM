...
export const PersonasView: React.FC<PersonasViewProps> = ({ workspaceId }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg transition"
          >
            + Novo Segmento
          </button>
          <button
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Carregando personas e segmentos...</div>
      ) : (
        <div className="space-y-6">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {segments.map((seg) => (
                  <span
                    key={seg.id}
                    className="px-3 py-1.5 rounded-lg bg-slate-800 text-slate-200 border border-slate-700 text-xs font-medium flex items-center gap-2"
                  >
          ...
          {personas.length === 0 ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-12 text-center">
              ...
              <h3 className="text-sm font-semibold text-white mb-1">Nenhuma Persona Cadastrada</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {personas.map((p) => (
                <div key={p.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-base font-bold text-white">{p.name}</h3>
                      <p className="text-xs text-indigo-300 font-medium">{p.role_title} • {p.segment || 'Geral'}</p>
                    </div>
                    <span className="w-8 h-8 rounded-full bg-indigo-500/20 text-indigo-300 font-bold flex items-center justify-center text-xs">
                      {p.name.charAt(0)}
                    </span>
                  </div>
                  ...
                  {p.jobs_to_be_done && p.jobs_to_be_done.length > 0 && (
                    <div className="space-y-1 pt-2 border-t border-slate-800">
                      <span className="text-[11px] font-bold text-slate-400 uppercase">Jobs To Be Done (JTBD)</span>
      ...
      {showPersonaModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-lg w-full text-slate-100 max-h-[90vh] overflow-y-auto">
            <h3 className="text-base font-bold text-white mb-4">Nova Persona do Produto</h3>
            <form onSubmit={handleCreatePersona} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Nome da Persona</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Cargo / Papel</label>
                  <input
                    type="text"
                    value={roleTitle}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Jobs To Be Done (1 por linha)</label>
                <textarea
                  value={jobsInput}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Dores (1 por linha)</label>
                <textarea
                  value={painsInput}
      ...
      {showSegmentModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-4">Novo Segmento de Clientes</h3>
            <form onSubmit={handleCreateSegment} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Nome do Segmento</label>
                <input
                  type="text"
                  value={segName}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Descrição & Critérios</label>
                <textarea
                  value={segDesc}
