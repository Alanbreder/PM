...
export const PRDView: React.FC<PRDViewProps> = ({ workspaceId }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Carregando PRDs e histórias...</div>
      ) : prds.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-12 text-center">
          <FileText className="w-10 h-10 text-slate-600 mx-auto mb-3" />
          <h3 className="text-sm font-semibold text-white mb-1">Nenhuma PRD Especificada</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-3 md:col-span-1">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Documentos PRD</h3>
            {prds.map((prd) => (
              <div
                key={prd.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="px-2 py-0.5 text-[10px] bg-indigo-500/20 text-indigo-300 rounded font-mono font-semibold uppercase">
                    v{prd.version || 1} • {prd.status}
                  </span>
                </div>
                <h4 className="text-xs font-bold text-white">{prd.title}</h4>
                {prd.roadmap_title && (
                  <span className="text-[10px] text-slate-400 flex items-center gap-1">
                    <Compass className="w-3 h-3 text-cyan-400" /> {prd.roadmap_title}
                  </span>
                )}
              </div>
            ))}
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="px-2 py-0.5 text-[10px] bg-indigo-500/20 text-indigo-300 rounded font-mono font-bold uppercase">
                      {selectedPrd.status}
                    </span>
                  ...
                  <h2 className="text-lg font-bold text-white">{selectedPrd.title}</h2>
                  <p className="text-xs text-slate-300 mt-2">{selectedPrd.summary}</p>
                </div>

                {/* Problem Statement */}
                {selectedPrd.problem_statement && (
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 space-y-1">
                    <h4 className="text-xs font-bold text-amber-400 uppercase">Problema Resolver</h4>
                    <p className="text-xs text-slate-200">{selectedPrd.problem_statement}</p>
                  </div>
                )}
                ...
                {selectedPrd.user_stories && selectedPrd.user_stories.length > 0 && (
                  <div className="space-y-3">
                    ...
                    {selectedPrd.user_stories.map((us: any, idx: number) => (
                      <div key={idx} className="bg-slate-800/80 p-4 rounded-lg border border-slate-700 space-y-2">
                          ...
                          <div className="pl-3 border-l-2 border-emerald-500/50 space-y-1 pt-1">
                            ...
                            <ul className="space-y-0.5">
                              {us.acceptanceCriteria.map((ac: string, acIdx: number) => (
                                <li key={acIdx} className="text-[11px] text-slate-300 flex items-center gap-1.5">
                ...
                {selectedPrd.technical_notes && (
                  <div className="space-y-1 pt-2 border-t border-slate-800">
                    <h4 className="text-xs font-bold text-slate-400 uppercase flex items-center gap-1.5">
                      <Code2 className="w-4 h-4 text-indigo-400" /> Requisitos & Anotações Técnicas
                    </h4>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-xl w-full text-slate-100 max-h-[90vh] overflow-y-auto">
            <h3 className="text-base font-bold text-white mb-4">Nova Especificação PRD</h3>
            <form onSubmit={handleCreatePrd} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Título da PRD</label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Vincular Item do Roadmap (Opcional)</label>
                <select
                  value={roadmapItemId}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Resumo Executivo</label>
                <textarea
                  value={summary}
              ...
              {/* User Story builder */}
              <div className="bg-slate-800/60 p-3.5 rounded-lg border border-slate-700/50 space-y-3">
                <span className="text-xs font-bold text-emerald-400 block uppercase">História de Usuário (User Story)</span>
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <label className="block text-[10px] text-slate-300 mb-1">Como (Persona)</label>
                    <input
                      type="text"
                      value={asA}
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-300 mb-1">Eu Quero (Ação)</label>
                    <input
                      type="text"
                      value={iWant}
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-300 mb-1">Para Que (Benefício)</label>
                    <input
                      type="text"
                      value={soThat}
                ...
                <div>
                  <label className="block text-[10px] text-slate-300 mb-1">Critérios de Aceite (1 por linha)</label>
                  <textarea
                    value={criteriaInput}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Notas Técnicas & APIs</label>
                <textarea
                  value={techNotes}
                  onChange={(e) => setTechNotes(e.target.value)}
                  rows={2}
                  placeholder="Rate limit, endpoints, componentes envolvidos..."
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white outline-none font-mono"
                />
              </div>
