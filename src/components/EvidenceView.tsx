    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-sm font-semibold text-zinc-100 mb-1">Nenhuma evidência encontrada</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {evidences.map((ev) => (
            <div key={ev.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
              <div className="flex items-center justify-between text-[10px] text-zinc-400">
                <span className="font-semibold text-military-400">Fonte: {ev.source || 'Pesquisa'}</span>
              ...
              <p className="text-xs text-zinc-200 leading-relaxed font-mono bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                "{ev.content}"
              </p>
              {ev.tags && ev.tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {ev.tags.map((tag, i) => (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] bg-zinc-800 text-zinc-300 border border-zinc-700">
