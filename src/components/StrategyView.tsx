...
export const StrategyView: React.FC<StrategyViewProps> = ({ workspaceId }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Carregando estratégia e OKRs...</div>
      ) : objectives.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-12 text-center">
          <Target className="w-10 h-10 text-slate-600 mx-auto mb-3" />
          <h3 className="text-sm font-semibold text-white mb-1">Nenhum Objetivo Cadastrado</h3>
        ...
        <div className="space-y-6">
          {objectives.map((obj) => (
            <div key={obj.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-800">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 text-[10px] bg-indigo-500/20 text-indigo-300 rounded font-mono font-bold">
                      {obj.timeframe}
                    </span>
                    <span className="text-xs text-slate-400 font-medium">• {obj.owner_name}</span>
                  </div>
                  <h3 className="text-base font-bold text-white mt-1">{obj.title}</h3>
                  {obj.description && <p className="text-xs text-slate-300 mt-1">{obj.description}</p>}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-md transition flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 text-xs font-medium rounded-md transition flex items-center gap-1.5 border border-indigo-500/30"
                  >
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Resultados-Chave (KRs)</h4>
                {(!obj.key_results || obj.key_results.length === 0) ? (
                  <p className="text-xs text-slate-500 italic">Nenhum Key Result vinculado a este objetivo.</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {obj.key_results.map((kr: any) => (
                      <div key={kr.id} className="bg-slate-800/60 border border-slate-700/50 rounded-lg p-3.5 space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          <span className="text-xs font-bold text-white">{kr.title}</span>
      ...
      {showObjModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-4">Novo Objetivo Estratégico (OKR)</h3>
            <form onSubmit={handleCreateObjective} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Título do Objetivo</label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Descrição & Racional</label>
                <textarea
                  value={description}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Timeframe / Quarter</label>
                  <input
                    type="text"
                    value={timeframe}
      ...
      {showKrModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-4">Adicionar Key Result (KR)</h3>
            <form onSubmit={handleCreateKeyResult} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Título do Key Result</label>
                <input
                  type="text"
                  value={krTitle}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Valor Inicial</label>
                  <input
                    type="number"
                    value={initialVal}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Valor Meta (Target)</label>
                  <input
                    type="number"
                    value={targetVal}
      ...
      {showLinkModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full text-slate-100">
            <h3 className="text-base font-bold text-white mb-4">Vincular Oportunidade ao Objetivo</h3>
            <form onSubmit={handleLinkOpportunity} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Selecione a Oportunidade</label>
                <select
                  value={linkOppId}
