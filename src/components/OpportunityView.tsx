    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-sm font-semibold text-zinc-100 mb-1">Nenhuma oportunidade registrada</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {opportunities.map((op) => (
            <div key={op.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
              ...
              <div className="flex items-center justify-between text-[11px] text-zinc-400 pt-2 border-t border-zinc-800">
                ...
                <span>Esforço: <strong className="text-military-400">{op.effort}</strong></span>
                <span>Status: <strong className="text-military-400">{op.status}</strong></span>
              </div>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 max-w-lg w-full text-zinc-100">
            <h3 className="text-base font-bold text-zinc-100 mb-4">Nova Oportunidade de Solução</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Valor Gerado</label>
                  <select
                    value={value}
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Esforço Estimado</label>
                  <select
                    value={effort}
