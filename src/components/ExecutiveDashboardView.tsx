...
export const ExecutiveDashboardView: React.FC<ExecutiveDashboardProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-500/20 p-6 rounded-xl shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">Visão Geral do Workspace</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg transition flex items-center gap-2 shadow-sm"
          >
            <BrainCircuit className="w-4 h-4" /> Diagnóstico IA
          </button>
          <button
            onClick={loadDashboard}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400">Saúde do Discovery</span>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400">Objetivos Estratégicos</span>
          ...
          <div>
            ...
            <span className="text-xs text-indigo-300 font-medium">Ativos no Quarter</span>
          </div>
          <button
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400">Experimentos em Curso</span>
          ...
          <div>
            ...
            <span className="text-xs text-amber-300 font-medium">Em Execução/Validação</span>
          </div>
          <button
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400">Itens em Roadmap</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              <Target className="w-4 h-4 text-indigo-400" />
              <h3 className="text-sm font-bold text-white">Objetivos Estratégicos (OKRs)</h3>
            </div>
            <button
              onClick={() => onNavigateTab('strategy')}
          ...
          {strategic_objectives?.length === 0 ? (
            ...
            <div className="space-y-3">
              {strategic_objectives?.slice(0, 3).map((obj: any) => (
                <div key={obj.id} className="bg-slate-800/60 rounded-lg p-3.5 space-y-2 border border-slate-700/50">
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-xs font-bold text-white leading-snug">{obj.title}</span>
                    <span className="px-2 py-0.5 text-[10px] bg-indigo-500/20 text-indigo-300 rounded font-mono shrink-0">
                      {obj.timeframe}
                    </span>
                  </div>
                  {obj.description && <p className="text-[11px] text-slate-300 line-clamp-1">{obj.description}</p>}
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                    <span>{obj.key_results?.length || 0} Key Results vinculados</span>
                    <span className="font-semibold text-emerald-400">{obj.progress || 0}% Concluído</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Top Oportunidades Priorizadas</h3>
            </div>
            <button
          ...
          {top_opportunities?.length === 0 ? (
            ...
            <div className="space-y-3">
              {top_opportunities.slice(0, 4).map((op: any, index: number) => (
                <div key={op.id} className="bg-slate-800/60 rounded-lg p-3 flex items-center justify-between gap-3 border border-slate-700/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 line-clamp-1">{op.description}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-xs font-extrabold text-amber-400">Score: {op.score || 0}</div>
                    <span className="text-[10px] text-slate-400 uppercase">{op.status || 'discovery'}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4 lg:col-span-1">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              <FlaskConical className="w-4 h-4 text-amber-400" />
              <h3 className="text-sm font-bold text-white">Experimentos Ativos</h3>
            </div>
            <button
              onClick={() => onNavigateTab('experiment')}
          ...
          {active_experiments?.length === 0 ? (
            <p className="text-xs text-slate-500 py-4 text-center">Nenhum experimento ativo no momento.</p>
          ) : (
            <div className="space-y-2.5">
              {active_experiments.slice(0, 3).map((exp: any) => (
                <div key={exp.id} className="p-3 rounded-lg bg-slate-800/40 border border-slate-700/40 space-y-1">
                  <div className="flex items-center justify-between text-xs font-semibold text-white">
                    <span>{exp.title}</span>
                    <span className="px-1.5 py-0.5 text-[9px] bg-amber-500/20 text-amber-300 rounded uppercase">
                      {exp.status}
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-400 line-clamp-1">{exp.metric}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4 lg:col-span-2">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              <Activity className="w-4 h-4 text-emerald-400" />
              <h3 className="text-sm font-bold text-white">Linha do Tempo de Atividades do Time</h3>
            </div>
            <span className="text-[10px] text-slate-400">Tempo Real</span>
          </div>

          {recent_activity?.length === 0 ? (
            <p className="text-xs text-slate-500 py-4 text-center">Nenhuma atividade recente gravada.</p>
          ) : (
            <div className="space-y-3">
              {recent_activity.slice(0, 5).map((act: any) => (
                <div key={act.id} className="flex items-start gap-3 text-xs">
                  ...
                  <div className="flex-1">
                    ...
                    <span className="text-slate-300">executou a ação </span>
                    <span className="px-1.5 py-0.5 bg-slate-800 text-indigo-300 rounded font-mono text-[10px]">
                      {act.action}
                    </span>{' '}
                    <span className="text-slate-400">em {act.entity_type}</span>
                  </div>
