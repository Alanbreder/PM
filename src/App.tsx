...
import { ExperimentsListView } from './components/ExperimentsListView';
...
export default function App() {
  ...
  const [experimentFilterHypothesisId, setExperimentFilterHypothesisId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadWorkspaceData = async () => {
      try {
        const [resR, resP, resO, resE, resH, resEx] = await Promise.allSettled([
          ...
          fetch('/api/experiments', { headers, signal }),
        ]);
        ...
        if (resEx.status === 'fulfilled' && resEx.value.ok) {
          const data = await resEx.value.json();
  ...
  const fetchExperiments = async () => {
    ...
    try {
      ...
      console.error('Erro ao listar experimentos:', err);
    } finally {
      setLoadingExperiments(false);
    }
  };
  ...
  const getBreadcrumbs = (): BreadcrumbItem[] => {
    ...
    switch (currentNavKey) {
      ...
      case 'opportunities':
        ...
        break;
      case 'hypotheses':
        items.push({ label: 'Hipóteses' });
        break;
      case 'experiments':
        items.push({ label: 'Experimentos' });
        break;
      case 'traceability':
    ...
    >
      ...
      {currentNavKey === 'hypotheses' && (
        <HypothesesListView
          ...
          onShowToast={showToast}
          onViewExperiments={(hypothesisId) => {
            setExperimentFilterHypothesisId(hypothesisId);
            handleNavigate('experiments');
          }}
        />
      )}

      {/* 6.5. Experiments View */}
      {currentNavKey === 'experiments' && (
        <ExperimentsListView
          ...
          showToast={showToast}
          activeWorkspaceId={activeWorkspaceId}
          initialHypothesisIdFilter={experimentFilterHypothesisId}
        />
      )}
